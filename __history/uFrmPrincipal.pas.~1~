...
begin
  ...
  begin
    MessageDlg('Local inválido!' + #13 + #13 + 'Apenas espaços vazios são válidos',mtError,[mbOk],0);
    exit;
  end;
...
begin
  ...
  if CheckLines or CheckCollumns or CheckDiagonals then
   if MessageDlg(winner + ' é o vencedor!!' + #13 + #13 + 'Recomeçar?',mtInformation,[mbYes, mbNo],0) = mrYes then
   begin
      ClearGame;
      DesenhaTela;
...
begin
  if JogoCheio then
    if MessageDlg('VELHA! Sem ganhadores Nesta Rodada' + #13 + #13 + 'Recomeçar?',mtInformation,[mbYes, mbNo],0) = mrYes then
    begin
      ClearGame;
      DesenhaTela;
      PEscolha.Visible := True;
